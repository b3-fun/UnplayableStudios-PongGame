...
const EditProfile = (props: Props) => {
    ...
    if (e.target.files && e.target.files[0]) {
      if (e.target.files[0].type.split('/')[0] !== 'image') {
        dispatch(
          newNotification({
            type: 'Error',
            message: 'Wrong avatar image type',
          }),
        );
  ...
  const changeUsername = (e: React.ChangeEvent<HTMLInputElement>) =>
    setUsername(e.target.value.replace(REGEX_ALPHANUM, ''));
  const changeFacebook = (e: React.ChangeEvent<HTMLInputElement>) =>
    setFacebook(e.target.value.replace(REGEX_ALPHANUM, ''));
  const changeDiscord = (e: React.ChangeEvent<HTMLInputElement>) =>
    setDiscord(e.target.value.replace(REGEX_ALPHANUM, ''));
  const changeInstagram = (e: React.ChangeEvent<HTMLInputElement>) =>
    setInstagram(e.target.value.replace(REGEX_ALPHANUM, ''));
  ...
  const submitProfile = () => {
    if (
      ...
      dispatch(
        newNotification({
          type: 'Error',
          message: 'Input too long / too short',
        }),
      );
    } else {
