export const chatReducer = (state: any, action: any) => {
  switch (action.type) {
    ...
    case 'REMOVE_FRIEND':
      return {
        ...state,
        newFriends: state.newFriends.filter((friend: any) => friend.id != action.data),
      };
    ...
    case 'REMOVE_GROUP':
      return {
        ...state,
        newGroups: state.newGroups.filter((group: any) => group.id != action.data),
      };
    ...
    case 'REMOVE_MEMBER':
      return {
        ...state,
        newMembers: state.newMembers.filter((member: any) => member.id !== action.data),
      };
    ...
    case 'REMOVE_ALL_GROUPS':
      return {
        ...state,
        allGroups: state.allGroups.filter((group: any) => group.id !== action.data),
      };
